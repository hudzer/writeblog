...
jQuery(function ($) {
    ...
    jQuery(document).on('click', 'input.select-img', function (event) {
        ...
        write_blog_file_frame = wp.media.frames.write_blog_file_frame = wp.media({
            button: {
        ...
        write_blog_file_frame.on('select', function () {

            var state = write_blog_file_frame.state('custom-theme-insert-image');
